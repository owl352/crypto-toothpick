...
function loadNative() {
    ...
    try {
        return require(`./${path.join('native', target, 'crypto_toothpick.node')}`);
    }
    catch (error) {
        ...
        console.error(`Failed to load native crypto-toothpick (${target}): ${reason}. Using WebAssembly instead Node-API`);
        return null;
    }
}
