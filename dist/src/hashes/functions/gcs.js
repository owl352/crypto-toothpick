 ...
 */
export function gcsMatchAny(filter, blockHash, items, params = {}) {
    return hashesProvider.hashes.gcsMatchAny(toBytes(filter), toBytes(blockHash), items.map(toBytes), params.p, params.m?.toString());
}
/**
 * Same as {@link gcsMatchAny}, keyed directly by the two 64-bit halves rather
 * than by the block hash they are derived from.
 */
export function gcsMatchAnyWithKeys(filter, k0, k1, items, params = {}) {
    return hashesProvider.hashes.gcsMatchAnyWithKeys(toBytes(filter), k0.toString(), k1.toString(), items.map(toBytes), params.p, params.m?.toString());
}
