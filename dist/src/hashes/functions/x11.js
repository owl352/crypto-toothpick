 ...
 */
export function x11Hash(header) {
 ...
 * @throws if the header is not exactly `X11_INPUT_LENGTH` bytes
 */
export function x11HashHex(header) {
 ...
 */
export function x11HashMany(headers) {
    return hashesProvider.hashes.x11HashMany(Array.isArray(headers) ? concatBytes(headers) : toBytes(headers));
}
