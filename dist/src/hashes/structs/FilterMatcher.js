...
export class FilterMatcher {
     ...
     */
    matchBlockMany(filters, blockHashes) {
        const payloads = filters.map(toBytes);
        const offsets = new Uint32Array(payloads.length + 1);
        for (let at = 0; at < payloads.length; at++) {
            offsets[at + 1] = offsets[at] + payloads[at].length;
        }
        return this._rawMatcher.matchBlockMany(concatBytes(payloads), offsets, Array.isArray(blockHashes) ? concatBytes(blockHashes) : toBytes(blockHashes));
    }
    /**
