 ...
 */
export function cfilterHeaderChain (
  prev: BytesLike,
  filterHashes: BytesLike | BytesLike[]
): Uint8Array {
  return hashesProvider.hashes.cfilterHeaderChain(
    toBytes(prev),
    Array.isArray(filterHashes) ? concatBytes(filterHashes) : toBytes(filterHashes)
  )
}
